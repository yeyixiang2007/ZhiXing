...
> 以 V1.0（7-10 天）为基线，按 10 天计划安排。

| 阶段 | 日期（2026） | 主要任务 | 产出物 | 负责人建议 |
|---|---|---|---|---|
| 需求与设计 | 04-03 | 明确范围、拆解模块、确定数据模型与接口 | 需求确认单、类图初稿 | 全员 |
| 核心建模 | 04-04 ~ 04-05 | 实现 `Vertex/Edge/CampusGraph` 与基础校验 | 领域模型代码、单元测试 | 成员A |
| 路径算法 | 04-06 ~ 04-07 | 实现 Dijkstra、不可达判断、禁行过滤 | 算法模块、样例测试数据 | 成员B |
| 地图管理 | 04-08 | 地点/道路增删改查、禁行设置接口 | MapService 模块 | 成员A |
| 持久化与权限 | 04-09 | 文件读写、管理员登录、角色权限控制 | Persistence/Auth 模块 | 成员C |
| 交互整合 | 04-10 | 控制台菜单、流程串联、异常提示完善 | 可运行主程序 | 成员D |
| 联调与测试 | 04-11 | 功能回归、性能测试、异常场景测试 | 测试报告、缺陷清单 | 全员 |
| 文档与答辩材料 | 04-12 | 完成课程设计报告、PPT、演示脚本 | 最终文档与演示包 | 全员 |

### 5.1 里程碑
1. M1（04-05）：图模型可用，基础数据操作通过。  
2. M2（04-07）：最短路径主流程跑通。  
3. M3（04-10）：系统端到端可演示。  
4. M4（04-12）：文档齐套，达到课程验收标准。
